     ...
     $root = Split-Path -Parent -Path $MyInvocation.MyCommand.Definition
        ...
        $dbs | % {
            ...
            Write-Progress -Activity "Upgrading Content database" -Status "$name ($prct %) $(Get-Date)" -PercentComplete $prct
            $counter++
        
            $wa = Get-SPWebApplication $_.WebApp

            Upgrade-SPContentDatabase -Name $name -WebApplication $wa -UseSnapshot -Confirm:$false
